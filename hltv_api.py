...
def top30teams():
    ...
    return teamlist
def get_teams():
    page = get_parsed_page("https://www.hltv.org/stats/teams?minMapCount=90") #minMapCount is number of played maps
    teams = page.find("table", {"class": "stats-table player-ratings-table"})
    teamlist = {}
    for team in teams.find_all("td", {"class": "teamCol-teams-overview"}):
        teamlist.update({ team.find('a').text : team.find('a')['href'][13:17] }) 
        #teamlist.append(newteam)
    return teamlist
    ...
    for match in matchdays:
        ...
        for getMatch in matchDetails:
            ...
            if (getMatch.find_all("td", {"class": "team-cell"})):
                matchObj['team1'] = getMatch.find_all("td", {"class": "team-cell"})[0].text.lstrip().rstrip()
                matchObj['team2'] = getMatch.find_all("td", {"class": "team-cell"})[1].text.lstrip().rstrip()
            else:
                matchObj['team1'] = None
                matchObj['team2'] = None
...
def get_results(team): 
    results = get_parsed_page("https://www.hltv.org/results?team="+team) #if delete $team then show all results
